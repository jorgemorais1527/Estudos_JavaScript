...
async function searchButtonClickHandle() {
  try {
    let url = `http://www.omdbapi.com/?apikey=${key}&t=${movieNameParameterGererator()}${movieYearParameterGererator()}`;
    console.log("URL: ", url);
...
function movieNameParameterGererator() {
  if (movieName.value === "") {
...
function movieYearParameterGererator() {
  if (movieYear.value === "") {
    return "";
  }
...
function updateUI(movieObject) {
  ...
  <button
     class="remover-button" onclick="{removeFilmFromList('${movieObject.imdbID}')}" ><i class="bi bi-trash"></i> Remover
  </button>
</article>`;
}
